...
import { useFinancial, type Budget, type BudgetItem } from "@/hooks/useFinancial";
...
export const EditBudgetModal = ({ open, onOpenChange, budget }: EditBudgetModalProps) => {
  const { patients } = usePatients();
  const { professionals } = useProfessionals();
  const { updateBudget } = useFinancial();
  ...
  // Preencher dados quando o orçamento mudar
  useEffect(() => {
    if (budget) {
      setFormData({
        ...
        notes: budget.notes || "",
        terms_conditions: budget.terms_conditions || "",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateBudget(budget.budget_id, budgetData);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Orçamento - {budget.budget_number}</DialogTitle>
          <DialogDescription>
              ...
              <div className="border rounded-lg overflow-hidden">
                ...
                <div className="divide-y">
                  {items.map((item, index) => (
                    <div key={item.item_id || index} className="grid grid-cols-12 gap-2 p-3 items-center">
